...
def v(tab_sym):
    ...
    variaveis = variaveis.drop_duplicates(subset=['lex', 'escopo', 'iniciacao'], keep='first')
    ...
    for _, row in variaveis.iterrows():
        ...
        df = tab_sym.loc[(tab_sym['lex'] == lex) & (tab_sym['escopo'] == escopo)]
        if len(df) > 1:
    ...
    for func in funcoes:
        if func == 'principal':
            ...
            if tabela_retorno.shape[0] == 0:
                print(error_handler.newError('ERR-RET-TIP-INCOMP', value=['inteiro']))
            ...
            chamada_funcao = tab_sym.loc[(tab_sym['lex'] == func) & (tab_sym['funcao'] == 'chamada_funcao')]
            declaracao_funcao = tab_sym.loc[(tab_sym['lex'] == func) & (tab_sym['funcao'] == '1')]
            ...
            if func == 'retorna':
                ...
                if len(chamada_funcao) > 0:
                    ...
                    if len(declaracao_funcao) > 0 and func != 'retorna':
                        print(error_handler.newError('WAR-NOT-USED-FUNC', value=func))
